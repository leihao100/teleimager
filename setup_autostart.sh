...
#   sudo systemctl status teleimager-test.service     # Check status
#   sudo journalctl -u teleimager-test.service -f     # View logs
#   sudo systemctl restart teleimager-test.service    # Restart service
#   sudo systemctl disable teleimager-test.service    # Disable auto-start
#
...
# Step 3: Test teleimager-test-server availability
echo "Checking teleimager-test-server command in environment '$CONDA_ENV'..."
source "$CONDA_PATH/etc/profile.d/conda.sh"
conda activate "$CONDA_ENV"

if ! command -v teleimager-test-server >/dev/null 2>&1; then
    echo "Error: teleimager-test-server command not found in '$CONDA_ENV'."
    exit 1
fi
echo "teleimager-test-server command found successfully."
...
SERVICE_NAME="teleimager-test.service"
SERVICE_FILE="/etc/systemd/system/$SERVICE_NAME"
CMD_WITH_ARGS="teleimager-test-server $USE_RS"
