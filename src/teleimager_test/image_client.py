...
class ImageClient:
    def __init__(self, host="192.168.123.164", request_port=60000, request_bgr: bool = False):
        ...
        try:
            ...
            # Depth streams are published by the server as raw uint16 (z16) bytes on their own
            # ports (RealSense only). Subscribe without BGR decoding; we reshape the raw bytes.
            for cam_topic in ('head_camera', 'left_wrist_camera', 'right_wrist_camera'):
                cam_cfg = self._cam_config.get(cam_topic, {})
                if cam_cfg.get('enable_depth', False) and cam_cfg.get('depth_port') is not None:
                    self._subscriber_manager.subscribe(self._host, cam_cfg['depth_port'], request_bgr=False)
        except Exception:
    ...
    # depth api
    # --------------------------------------------------------
    def _decode_depth(self, data: Optional[bytes], image_shape) -> Optional[np.ndarray]:
        """Reconstruct a HxW uint16 depth image from raw z16 bytes."""
        if data is None:
            return None
        try:
            depth = np.frombuffer(data, dtype=np.uint16)
            return depth.reshape(image_shape[0], image_shape[1])
        except Exception as e:
            logger_mp.warning(f"[ImageClient] Failed to decode depth frame: {e}")
            return None

    def _get_depth_frame(self, cam_topic: str) -> Optional[np.ndarray]:
        cam_cfg = self._cam_config.get(cam_topic, {})
        if not cam_cfg.get('enable_depth', False) or cam_cfg.get('depth_port') is None:
            return None
        tele = self._subscriber_manager.subscribe(self._host, cam_cfg['depth_port'], request_bgr=False)
        return self._decode_depth(tele.jpg, cam_cfg['image_shape'])

    def get_head_depth_frame(self) -> Optional[np.ndarray]:
        return self._get_depth_frame('head_camera')

    def get_left_wrist_depth_frame(self) -> Optional[np.ndarray]:
        return self._get_depth_frame('left_wrist_camera')

    def get_right_wrist_depth_frame(self) -> Optional[np.ndarray]:
        return self._get_depth_frame('right_wrist_camera')
...
def main():
    ...
    while running:
        ...
        # Depth preview (only when the server publishes it, e.g. a RealSense head).
        if cam_config['head_camera'].get('enable_depth', False):
            head_depth = client.get_head_depth_frame()
            if head_depth is not None:
                # Colorize the uint16 depth (mm) for visualization only.
                depth_vis = cv2.applyColorMap(
                    cv2.convertScaleAbs(head_depth, alpha=0.03), cv2.COLORMAP_JET)
                cv2.imshow("Head Depth", depth_vis)
